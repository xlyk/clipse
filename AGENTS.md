...
- **A committed scheduling pause is a hard claim barrier.** `dispatcher_control.desired_mode` is read inside the same `BEGIN IMMEDIATE` transaction as `ClaimReady` and `ClaimColumn`; `pause`/`drain` writes use the same transaction mode. A claim that commits first may finish (including same-claim `continue` turns); after pause commits every coder, reviewer, and git-operator claim returns `store.ErrSchedulingPaused` until resume. A completed drain leaves `desired_mode=paused`, so a replacement recovers any genuine crash orphan but schedules nothing before explicit resume.
...
- **One dispatcher per machine** (flock singleton). A first SIGINT/SIGTERM requests drain when work is active (or exits immediately when idle); a second signal forces the legacy immediate shutdown and logs the open run IDs. Worker timeout contexts remain rooted at `context.WithoutCancel(...)`, so dispatcher cancellation itself does not kill live workers. The local spawner's deadline path kills the complete worker process group before `Wait`, with a bounded `WaitDelay` for inherited pipes.
