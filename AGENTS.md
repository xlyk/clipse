...
- **Linear `SetState`**: the HTTP client authenticates by API key but does not yet resolve `contract.Column` → per-team Linear workflow-state id; wire that when the real board exists.
