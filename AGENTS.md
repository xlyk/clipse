...
- **openai_codex needs a pre-built model object**: a bare `openai_codex:*` spec string can't go straight into `create_cli_agent` — it routes through LangChain's `init_chat_model`, which raises `ValueError` on the DAC-only `openai_codex` provider (never registered with LangChain). `dac.build_coder_agent` special-cases that one provider: it pre-builds a `BaseChatModel` via `deepagents_code.config.create_model(...).model` and passes the object instead of the spec string; every other provider keeps the plain-string path untouched.
